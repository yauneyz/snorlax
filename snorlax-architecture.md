...
3. Service (privileged) reads the best stable identifier and stores it in its root/SYSTEM-owned
   data directory. Only if none is available does it generate a random secret, write
   `.talysman/key.bin`, and store the secret hash.
