...
const target =
  targetIdx !== -1 ? args[targetIdx + 1] : platformTarget(process.platform);
const cross = args.includes("--cross");
const WINDOWS_CROSS_TARGET = "x86_64-pc-windows-msvc";
...
const TARGETS = {
  win: {
    ...
    cargoEnv(crate) {
      return {
        ...process.env,
        WINDIVERT_PATH: resolve(crate, "vendor/windivert"),
      };
    },
    artifacts: [
      "talysman-svc.exe",
...
if (!cfg) {
  console.error(
    `Unsupported native target "${target}". Expected one of: ${Object.keys(TARGETS).join(", ")}`,
  );
  process.exit(1);
}
...
if (crossWindowsFromLinux) {
  run(
    "cargo",
    ["xwin", "build", "--release", "--target", WINDOWS_CROSS_TARGET],
    crate,
    cfg.cargoEnv(crate),
  );
} else {
...
const targetDir = resolve(
  crate,
  crossWindowsFromLinux
    ? `target/${WINDOWS_CROSS_TARGET}/release`
    : "target/release",
);
