...
export async function RetentionPanel({
  target,
  audience,
}: {
  ...
  const result = await queryRetention(target, audience);
  return (
    <PanelShell
      title="Retention cohorts"
      description="Protected-active windows: D1 1–2, D7 5–9, D30 27–33."
    >
      {!result.ok ? (
        <Unavailable message={result.message} />
      ) : result.rows.length === 0 ? (
        <p className="insights-muted">No install cohorts yet.</p>
      ) : (
