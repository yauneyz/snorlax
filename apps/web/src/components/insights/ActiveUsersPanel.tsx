  ...
  return <PanelShell title="Active users" description="Protected activity is at least 60 seconds of focus.">
    {!result.ok ? <Unavailable message={result.message} /> : !data ? <p className="insights-muted">No usage reported yet.</p> : <>
      <div className="insights-kpis">
        <div><span>DAU protected</span><strong>{data.dauProtected}</strong></div>
        <div><span>DAU UI</span><strong>{data.dauUi}</strong></div>
        <div><span>MAU protected</span><strong>{data.mauProtected}</strong></div>
        <div><span>30d installed base</span><strong>{data.installedBase30d}</strong></div>
      </div>
      <div className="insights-spark-table">{data.series.map((point) =>
