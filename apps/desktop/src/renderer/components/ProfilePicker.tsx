  ...
  return (
    <div className="absolute inset-0 z-20 flex items-center justify-center" role="dialog" aria-modal="true" aria-label="Switch profile">
      <button
        ...
        onClick={onClose}
        className="absolute inset-0 bg-[rgb(var(--color-background)/0.90)] backdrop-blur-md"
      />
      <div className="relative" style={{ width: BOX, height: BOX }}>
        <div className="absolute inset-[60px] rounded-full border border-dashed border-white/[0.08]" />
        <div className="absolute inset-0 rounded-full bg-[radial-gradient(circle,rgb(var(--color-signal)/0.07),transparent_68%)]" />
          ...
          return (
            <button
              ...
              className={cx(
                ...
                on
                  ? 'border-signal/30 bg-signal/[0.07] shadow-[0_0_26px_rgb(var(--color-signal)/0.14)]'
                  : 'border-white/[0.09] bg-[rgb(var(--color-panel)/0.9)] shadow-[0_8px_24px_rgb(var(--color-black)/0.6)] hover:border-white/20 hover:bg-white/[0.05]',
              )}
              style={{
                width: NODE,
                height: NODE,
        ...
        <div className="absolute left-1/2 top-1/2 flex h-20 w-20 -translate-x-1/2 -translate-y-1/2 flex-col items-center justify-center gap-1 rounded-full border border-white/[0.08] bg-[rgb(var(--color-background)/0.85)]">
