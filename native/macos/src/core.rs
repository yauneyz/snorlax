...
impl Core {
    ...
    fn recover(&mut self, code: &str) -> Result<(), RpcError> {
        // The recovery hash may have been (re)generated by `svcctl gen-code` after this daemon
        // loaded its store; refresh it from disk so a valid code is never refused stale.
        if let Some(recovery) = SecureStore::load().recovery {
            self.store.recovery = Some(recovery);
        }
